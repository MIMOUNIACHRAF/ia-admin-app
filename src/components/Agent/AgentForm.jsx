...
export default function AgentForm({ onSubmit, initialData, templates }) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setSelectedTemplates(initialData.templates.map(t => t.id));
    } else {
      setNom(""); setDescription(""); setTypeAgent("trad"); setSelectedTemplates([]);
    }
  }, [initialData]);

  const handleSubmit = e => {
    e.preventDefault();
    onSubmit({ nom, description, type_agent: typeAgent, template_ids: selectedTemplates });
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 border p-4 rounded bg-gray-50">
      ...
      <div>
        ...
        <select multiple value={selectedTemplates} onChange={e => setSelectedTemplates([...e.target.selectedOptions].map(o => parseInt(o.value)))} className="w-full border px-3 py-2 rounded">
          {templates.map(t => (
